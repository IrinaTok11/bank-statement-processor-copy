"""
Merge multiple business bank statements into one standardized Transactions table and create
supporting analysis sheets (Findings & Alerts, Analytics Dashboard, Config & Thresholds).

This project is designed for synthetic/demo data and intentionally avoids real PII.
"""
import pandas as pd
import os
...
def configure_logging(verbose: bool = False) -> None:
    ...
    The default output format is message-only to keep logs readable.
    """
...
def patch_excel_app_metadata(xlsx_path: Path, application: str = "Microsoft Excel", app_version: str = "16.0300") -> None:
    """Patch docProps/app.xml to set Application/AppVersion to Excel-like values.

    Why this exists:
    - Workbooks written via openpyxl typically show an "Application" string like
      "Microsoft Excel Compatible / Openpyxl X.Y.Z" in docProps/app.xml.
    - For an automation project this is normal, but if you want the workbook to
      look closer to an Excel-saved file, you can patch that field.

    Notes:
    - This is a cosmetic metadata change only. It does not alter worksheet data.
    - The function edits the XLSX (ZIP) in-place in an atomic, safe way.
    """
...
def detect_bank_format(file_path):
    """
    Heuristic bank format detector for the formats defined in BankStatementConfig.
    
    Process:
    1. Extract bank name from filename (first word before underscore)
    2. Auto-find header row (looks for Date + Description + amount columns)
    3. Auto-detect format type (Debit/Credit separate vs Amount signed)
    4. Build column mapping
    5. Detect actual sheet name
    
    Args:
        file_path: Path to the Excel file
        
    Returns:
        dict: Bank configuration with:
            - 'bank': Bank name from filename
            - 'sheet': Actual sheet name from Excel file
            - 'header': Header row index
            - 'format_type': 'debit_credit' or 'amount_signed'
    ...
    try:
        # Step 1: Extract bank name from filename
        filename = os.path.basename(file_path)
        bank_name = filename.split('_')[0]  # First word before underscore
        
        # Step 1.5: Get actual sheet name from Excel file
        wb = openpyxl.load_workbook(file_path, read_only=True)
        actual_sheet_name = wb.sheetnames[0]  # Use first sheet
        
        # CRITICAL WARNING: Check for multi-sheet files
        ...
        # Step 2: Read file to find header
...
def normalize_date_column(df, column_name):
    ...
    Converts values to datetime, removes time component.
    """
...
def normalize_amount_column(df, column_name):
    """
    Normalize amount columns (debit/credit).
    Removes spaces, commas, converts to numbers.
    """
    if column_name in df.columns:
        # Convert to string, remove spaces and commas
...
def detect_findings(df, return_run_stats=False):
    """
    Detect specific actionable findings that require review.
    Returns structured findings with priority levels, evidence, and traceability.
    
    This implements rule-based detection for common fraud patterns and 
    business risks, based on forensic accounting practices.
    """
    findings = []
    run_stats = {}
    alert_id = 1
    ...
    for i in range(len(df_sorted) - 1):
        ...
        if (current['_vendor_key'] == next_row['_vendor_key'] and
            ...
            findings.append({
                ...
                'Amount_Type': 'Transaction Amount',  # NEW: clarify what Amount means
                'Date_1': current['Date'],
    ...
    for (date, vendor), group in df_debits.groupby([df_debits['Date'], '_vendor_key']):
        if len(group) >= RULES['split_payments']['params']['min_count'] and group['Debit'].sum() > RULES['split_payments']['params']['min_total']:
            ...
            findings.append({
                ...
                'Amount_Type': 'Total Paid (Multiple Transactions)',  # NEW: clarify this is sum
                'Date_1': date,
                'Date_2': None,
            ...
            if len(amounts) >= 2:
                ...
                if baseline_amount > RULES['vendor_concentration_growth']['params']['baseline_min'] and latest_amount > baseline_amount * RULES['vendor_concentration_growth']['params']['growth_factor']:
                    ...
                    findings.append({
                        ...
                        'Amount_Type': 'Baseline-to-Latest Growth (Delta)',  # NEW: clarify this is delta, not total
                        ...
                        'Txn_IDs': txn_id_summary,  #  Now with concrete Txn_IDs
...
def classify_operation_type(df, description_column, transaction_type_column=None):
    ...
    Creates a new column 'Operation_Type' with classification.
    
    Args:
        df: DataFrame with data
        description_column: name of the column with transaction description
        transaction_type_column: optional column with transaction type (for Chase)
    """
...
def process_excel_file(file_path, output_columns):
    """
    Process a single Excel file with Heuristic automatic bank format detection.
    
    Works with ANY bank statement format:
    - Auto-detects bank name from filename
    - Auto-finds header row
    - Auto-detects format (Debit/Credit vs Amount signed)
    - Auto-maps columns
    
    Args:
        file_path: Path to the Excel file
        output_columns: List of required output column names
        
    Returns:
        DataFrame or None if processing failed
    """
    try:
        # Step 1: Automatic format detection
        ...
        # Step 2: Read data from header row
        df = pd.read_excel(file_path, header=header_row)
        
        # CRITICAL FIX: Capture original Excel row numbers BEFORE any filtering
        ...
        # Step 3: Filter out non-transaction rows
        initial_rows = len(df)
        ...
        # Step 5: Handle Debit/Credit vs Amount format
        if format_type == 'debit_credit':
        ...
        # Step 6.5: Filter out rows with empty Description (service rows, spacing, etc)
        ...
        # Step 7: Convert Amount column to Debit/Credit if needed
        ...
        # Step 8: Select only the columns we need
        # CRITICAL: Save _excel_row_original before it gets dropped
        ...
        # CRITICAL FIX: Use pre-calculated Excel row numbers (captured before filtering)
        ...
        # Step 11: Clean vendor names
        if 'Description' in df.columns:
            df = normalize_company_name(df, 'Description')
        
        # Step 12: Classify operations
        if 'Description' in df.columns:
...
def benford_law_analysis(df):
    """
    Apply Benford's Law to detect potential data manipulation.
    Returns analysis results and comparison with expected distribution.
    """
    # Get all debit amounts - EXCLUDE zeros and NaN
...
def main():
    ...
    OUTPUT_COLUMNS = [
        'Txn_ID',           # NEW: Unique transaction identifier
        'Date',
        'Description',      # NEW: Raw description (as in statement)
        ...
        'Direction',
        'Bank',             # Source bank
        'Source_File',      # NEW: Originating file
        'Source_Sheet',     # NEW: Sheet name in Excel file
        'Source_Row',       # NEW: Original row number in Excel
        'Amount_Signed'     # NEW: Single column (Credit - Debit)
    ]
    ...
    # CRITICAL FIX: Sort with stable tie-breakers to ensure deterministic Txn_ID assignment
